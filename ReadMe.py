...
# 使用python进行程序编写时，经常会使用第三方模块包。这种包我们可以通过python setup install 进行安装后，
# 通过import XXX或from XXX import yyy 进行导入。不过如果是自己遍写的依赖包，又不想安装到python的相应目录，
# 可以放到本目录里进行import进行调用；为了更清晰的理清程序之间的关系，例如我们会把这种包放到lib目录再调用。
# 本篇就针对常见的模块调用方法汇总下。
# 一、同级目录下的调有
# 程序结构如下：
#
...
if __name__ == '__main__':
    # 只有当前py执行的时候，才执行下面的方法，获取打印，否则不执行下面的方法
    print("Hello")